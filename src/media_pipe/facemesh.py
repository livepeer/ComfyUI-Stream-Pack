...
class FaceMeshUtils:
    ...
    def __init__(self):
        ...
        self.f_thick = 2
        self.f_rad = 1
        self.right_iris_draw = self.mp_drawing.DrawingSpec(color=(10, 200, 250), thickness=self.f_thick, circle_radius=self.f_rad)
        self.right_eye_draw = self.mp_drawing.DrawingSpec(color=(10, 200, 180), thickness=self.f_thick, circle_radius=self.f_rad)
        self.right_eyebrow_draw = self.mp_drawing.DrawingSpec(color=(10, 220, 180), thickness=self.f_thick, circle_radius=self.f_rad)
        self.left_iris_draw = self.mp_drawing.DrawingSpec(color=(250, 200, 10), thickness=self.f_thick, circle_radius=self.f_rad)
        self.left_eye_draw = self.mp_drawing.DrawingSpec(color=(180, 200, 10), thickness=self.f_thick, circle_radius=self.f_rad)
        self.left_eyebrow_draw = self.mp_drawing.DrawingSpec(color=(180, 220, 10), thickness=self.f_thick, circle_radius=self.f_rad)
        self.mouth_draw = self.mp_drawing.DrawingSpec(color=(10, 180, 10), thickness=self.f_thick, circle_radius=self.f_rad)
        self.head_draw = self.mp_drawing.DrawingSpec(color=(10, 200, 10), thickness=self.f_thick, circle_radius=self.f_rad)
...
class FaceMeshDrawNode:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                ...
                "mesh_color": (["green", "red", "blue", "yellow", "white"], {
                    "default": "green",
                    "description": "Color of the face mesh lines"
                }),
            },
        }
    ...
    def draw_facemesh(self, image, facemesh_data, draw_contours, mesh_thickness, mesh_color):
        ...
        for b in range(batch_size):
            ...
            output_img_rgb = self.utils.draw_facemesh_on_image(img_rgb, results, thickness=int(mesh_thickness))
...
class FaceMeshMaskNode:
    ...
    def INPUT_TYPES(s):
        ...
        return {
            "required": {
                ...
                "region": (regions, {
                    "default": "mouth",
                    "description": "Facial region to mask"
                }),
                "fill_region": ("BOOLEAN", {
                    "default": True,
    ...
    def create_mask(self, facemesh_data, face_index, region, fill_region, mask_value, dilation_pixels, blur_radius):
        ...
        for b in range(batch_size):
            ...
            if not current_facemesh_data["face_detected"] or face_index >= current_facemesh_data["num_faces_detected"]:
            ...
            if len(region_indices) > 0:
                ...
                region_landmarks = np.array([landmarks_array[i] for i in region_indices if i < len(landmarks_array)])
                
                if len(region_landmarks) > 2:  # Need at least 3 points to create a contour
                    # Convert normalized coordinates to pixel coordinates
                    points = np.zeros((len(region_landmarks), 2), dtype=np.int32)
                    points[:, 0] = (region_landmarks[:, 0] * w).astype(np.int32)  # x coordinates
                    points[:, 1] = (region_landmarks[:, 1] * h).astype(np.int32)  # y coordinates
                    ...
                    elif (region == "whole_face" or region == "face_oval") and fill_region:
...
class FaceMeshNode:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                ...
                "draw_mesh": ("BOOLEAN", {
                    "default": True,
                    "description": "Draw face mesh on output image"
                }),
                "draw_contours": ("BOOLEAN", {
                    "default": True,
                    "description": "Draw contour lines connecting mesh points"
    ...
    def detect_facemesh(self, image, static_image_mode, max_num_faces, refine_landmarks, 
                       min_detection_confidence, min_tracking_confidence,
                       draw_mesh, draw_contours, mesh_color, mesh_thickness):
        ...
        for b in range(batch_size):
            ...
            if draw_mesh and facemesh_data["face_detected"]:
                ...
                empty = self.utils.draw_facemesh(img_rgb, results, thickness=int(mesh_thickness))
